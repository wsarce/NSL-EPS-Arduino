...
bool EPS::radio(byte *packet)
{
  int tries = EPS_TRIES;
  while (!eps_radio_send(packet))
  {
...
bool EPS::eps_radio_send(byte *packet)
{
  eps_serial.write(packet, EPS_BYTES);
  byte ack[EPS_BYTES];
...
bool EPS::heartbeat(byte *packet)
{
  int tries = EPS_TRIES;
  while (!eps_heartbeat_send(packet))
  {
    if (tries-- <= 0) return false;
  }
  return true;
...
bool EPS::eps_heartbeat_send(byte *packet)
{
  eps_serial.write(packet, EPS_BYTES);
  byte ack[EPS_BYTES];
  memset(ack, 0, EPS_BYTES);
